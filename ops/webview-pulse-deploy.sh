...
backup="/srv/backups/gpunode/webview-pulse-$stamp"
release="/srv/releases/gpunode/$stamp-${sha:0:12}-webview-pulse"
...
rollback(){
  ...
  if ((activated)); then
    ...
    ln -sfn "$previous" "$root/current-webview-pulse-rollback"
    mv -Tf "$root/current-webview-pulse-rollback" "$root/current"
...
[[ -f $release/STELLAR_UPDATE.md && -f $release/frontend/src/stellar.css && -f $release/frontend/src/motion.ts ]] || fail 'В GitHub ещё нет обновления мерцания.'
[[ -f $release/WEBVIEW_PULSE_UPDATE.md ]] || fail 'В GitHub ещё нет исправления Telegram WebView.'
...
runuser -u deploy -- env PATH="$PATH" NODE_OPTIONS=--max-old-space-size=384 bash -c '
 ...
 npm test
 npm run build
 grep -Rqs 'LIVE NEURAL CORE' frontend/dist/assets/*.js
' _ "$release"
...
PY
chmod 0644 "$service"
ln -sfn "$release" "$root/current-webview-pulse-next"
mv -Tf "$root/current-webview-pulse-next" "$root/current"
systemctl daemon-reload
...
for asset in "${assets[@]}"; do
 ...
 if [[ $asset == *.js ]] && grep -q 'LIVE NEURAL CORE' "$target"; then bundle_marker=1; fi
done
((bundle_marker)) || fail 'HTTPS отдал старый JavaScript без исправления WebView.'
...
trap - ERR INT TERM
printf '\nГОТОВО: https://31.77.226.26/\nОбновление: Telegram WebView lifecycle, удаление старой паузы, яркое локальное мерцание ядра\nКоммит: %s\nБэкап: %s\nКнопка @Videomarket_GPUbot и HTTPS сохранены. Полностью закройте и заново откройте приложение.\n' "$sha" "$backup"
