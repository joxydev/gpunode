...
backup="/srv/backups/gpunode/agreement-$stamp"
release="/srv/releases/gpunode/$stamp-${sha:0:12}-agreement"
...
rollback(){
  ...
  if ((activated)); then
    ...
    ln -sfn "$previous" "$root/current-agreement-rollback"
    mv -Tf "$root/current-agreement-rollback" "$root/current"
...
PY
chmod 0644 "$service"
ln -sfn "$release" "$root/current-agreement-next"
mv -Tf "$root/current-agreement-next" "$root/current"
systemctl daemon-reload
