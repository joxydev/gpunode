...
try{
 ...
 }else if(d.metadata?.paymentMode!=='GASLESS'||!d.metadata?.gasless?.relaySubmittedAt||!d.metadata?.gasless?.fee||d.wallet_version!=='W5'||! /^[a-f0-9]{64}$/.test(d.public_key||''))throw Error('This is not a confirmed W5 gasless canary payment');
 process.stdout.write(JSON.stringify({invoiceId:d.invoice_id,userId:d.user_id,amountMicros:d.amount,txHash:d.tx_hash,traceId:d.trace_id,senderWallet:d.sender_address,mode,ledgerEntries:ledger.n,legacyEntries:entry.n,network:'TON Mainnet',feeMicros:mode==='gasless'?d.metadata.gasless.fee:undefined},null,2)+'\n');
