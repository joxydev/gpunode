...
backup="/srv/backups/gpunode/stellar-$stamp"
release="/srv/releases/gpunode/$stamp-${sha:0:12}-stellar"
...
rollback(){
  ...
  if ((activated)); then
    ...
    ln -sfn "$previous" "$root/current-stellar-rollback"
    mv -Tf "$root/current-stellar-rollback" "$root/current"
...
PY
chmod 0644 "$service"
ln -sfn "$release" "$root/current-stellar-next"
mv -Tf "$root/current-stellar-next" "$root/current"
systemctl daemon-reload
