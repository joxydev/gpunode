...
export default function Market({onRequest}:{onRequest:(node:NodeSpec)=>void}) {
  ...
  return <section className="market" aria-labelledby="market-heading">
    ...
    <div className="market-grid" aria-busy={loading}>{loading?Array.from({length:2},(_,i)=><div className="market-skeleton" key={i} aria-hidden="true"><div/><span/><span/></div>):nodes.map(node=><article key={node.id} className={`market-card ${node.tier>=3?'premium':''} ${node.availability==='SOLD_OUT'?'sold-out':''}`}>
