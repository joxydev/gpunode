...
export default function BrowserLogin({onSuccess}:{onSuccess:(token:string)=>Promise<void>}){
 ...
 useEffect(()=>{
  ...
  const poll=async()=>{
   ...
   try{if(approvedToken){await onSuccess(approvedToken);return;}
    ...
    if(result.status==='DENIED'||result.status==='EXPIRED'){setError(result.status==='DENIED'?'Вход отклонён в Telegram.':'Срок запроса истёк.');setEnded(true);return;}
    setError('');
 ...
 return <><span className="eyebrow">TELEGRAM LOGIN</span><h2>Вход в аккаунт</h2><p>Откройте бота, нажмите «Запустить» и подтвердите вход с этим кодом. Затем вернитесь в эту вкладку.</p>{challenge&&!ended&&<><div className="browser-login-code" aria-label="Код запроса">{challenge.code}</div><a className="primary wide" href={challenge.url} target="_blank" rel="noopener noreferrer">Подтвердить через Telegram</a><p className="fine">Запрос действует 5 минут. Не подтверждайте вход по чужим ссылкам.</p><p role="status">Ожидаем подтверждение…</p></>}{!challenge&&!ended&&<p role="status">Создаём запрос…</p>}{error&&<p role="alert">{error}</p>}{ended&&<button className="secondary wide" onClick={()=>setGeneration(n=>n+1)}>Начать вход заново</button>}</>;
}
