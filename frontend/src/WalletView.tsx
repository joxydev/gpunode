...
const labels:Record<string,string>={PENDING:'Ожидаем перевод',DETECTED:'Обнаружен',CONFIRMED:'Подтверждён',CREDITED:'Зачислен',EXPIRED:'Время счёта истекло',FAILED:'Ошибка',REJECTED:'Отклонён',MANUAL_REVIEW:'Ручная проверка'};

export default function WalletView({account,onRefresh,onLogin}:{account:Account|null;onRefresh:()=>Promise<void>;onLogin:()=>void}){
