...
export type RequestItem={id:string;nodeId:string;profile:string;workload:string;paymentStatus:'WAITING'|'PAID'|'TEST_CREDIT';isTestOrder:boolean;testPriceUsdt?:string|null;userUnread:boolean;status:string;decision?:string|null;closureReason?:string|null;closedAt?:string|null;createdAt:string;updatedAt:string;equipment?:{id:string;name:string};user?:{id:string;name:string;username?:string|null}};
...
export type Account={user:{id:string;name:string;username?:string|null;isOwner:boolean;preferredLanguage?:'ru'|'en'|'ro'|null};agreement:{version:string;accepted:boolean;acceptedAt:string|null};offer:{number:string;version:string;documentSha256:string;accepted:boolean;acceptedAt:string|null};balance:string;testBalance:string;testEntries:{id:string;amount:string;kind:string;reason:string;createdAt:string}[];testAssets:{id:string;requestId:string;status:'AWAITING_ALLOCATION';priceUsdt:string;termDays:number;rateBps:number;approvedAt:string;node:{name:string;chip:string;tflopsPower:number;workload:string;imageUrl:string}}[];earnedToday:null;selectedTariff:OfferTariff|null;journey:JourneyStep[];activeNodes:{id:string;status:string;createdAt:string;expiresAt:string;node:{name:string}}[];requests:RequestItem[];tickets:Ticket[];notifications:{unreadSupport:number;unreadOrders:number;pendingTestOrders:number};referrals:{total:number;active:number;rewardConfigured:boolean;items:ReferralItem[]};entries:{id:string;amount:string;kind:string;createdAt:string}[];paymentsEnabled:boolean;accrualEnabled:boolean;updatedAt:string};
