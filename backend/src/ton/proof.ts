...
const contracts=[
 {code:WalletContractV5R1.create({workchain:0,publicKey:empty}).init.code,extract:(cell:Cell)=>{const s=cell.beginParse();s.loadBit();s.loadUint(32);s.loadUint(32);return s.loadBuffer(32)}},
 {code:WalletContractV4.create({workchain:0,publicKey:empty}).init.code,extract:(cell:Cell)=>{const s=cell.beginParse();s.loadUint(32);s.loadUint(32);return s.loadBuffer(32)}},
 {code:WalletContractV3R2.create({workchain:0,publicKey:empty}).init.code,extract:(cell:Cell)=>{const s=cell.beginParse();s.loadUint(32);s.loadUint(32);return s.loadBuffer(32)}}
];
export async function verifyTonProof(input:ProofInput,expectedPayload:string,config:Config,center:Pick<TonCenter,'publicKey'>,now=Date.now()):Promise<string>{
 ...
 for(const known of contracts)if(state.code?.equals(known.code)&&state.data){key=known.extract(state.data);break;}
