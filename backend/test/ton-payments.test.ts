...
import {tonConfig,usdtUnits,attachForUser,structuredForUser,paymentCanary,DEFAULT_MASTER,DEFAULT_TREASURY} from '../src/ton/config.js';
...
test('TON Proof binds public key, stateInit, nonce, domain, network and time',async()=>{
 ...
 const data=wallet.init.data.beginParse();data.loadBit();data.loadUint(32);const walletId=data.loadUint(32).toString();
 assert.deepEqual(await verifyTonProof(input,payload,config,noGetter),{address:wallet.address.toRawString(),publicKey:key.toString('hex'),walletVersion:'W5',walletId});
...
test('valid V4 TON Proof persists a verified key and never claims W5',async()=>{
 ...
 assert.deepEqual(await verifyTonProof(input,payload,config,{publicKey:async()=>{throw Error('unexpected getter')}}),{address:wallet.address.toRawString(),publicKey:key.toString('hex'),walletVersion:'V4R2',walletId:null});
});
...
test('USDT is exact base units, transaction is Jetton transfer with tagged invoice',()=>{
 ...
 assert.equal(attachForUser(configured,'6662169510'),toNano('0.05'));
 ...
 assert.equal(structuredForUser(configured,'6662169510'),false,'test account smoke must use measurable raw attach');
 assert.equal(structuredForUser(configured,'other'),true);
 assert.equal(paymentCanary(configured,'112233'),true);
 assert.equal(paymentCanary(configured,'6662169510'),true);
 assert.equal(paymentCanary(configured,'9999999999'),false);
 assert.equal(paymentCanary(configured,'@VerstakPR'),false,'username alone never confers test access');
 for(const ids of [',6662169510','6662169510,','6662169510,6662169510','@VerstakPR',' 6662169510','6662169510;2'])assert.throws(()=>tonConfig({TON_PAYMENT_TEST_TELEGRAM_IDS:ids}));
...
test('invoice migration accepts cancellation and still constrains duplicate transactions',async()=>{
 ...
 try{
  for(const name of ['202609130001_initial','202609130002_market','202609150001_browser_login','202609150002_user_agreement','202609150003_managed_requests','202609190001_owner_support','202609220001_public_offer','202609220002_owner_center','202609230001_user_language','202609230002_test_cycle','202609240001_ton_usdt_deposits','202609240002_cancel_invoice','202609250001_ton_proof_wallet_identity','202609260001_w5_wallet_id'])await db.exec(migration(name));
  await db.exec("INSERT INTO \"User\"(id,name) VALUES ('2','User');");
  await db.exec(`INSERT INTO user_wallets(user_id,address,public_key,wallet_version,wallet_id) VALUES ('2','0:${'0'.repeat(63)}1','${'a'.repeat(64)}','W5','2147483648')`);
  const verified=await db.query<{public_key:string;wallet_version:string;wallet_id:string}>("SELECT public_key,wallet_version,wallet_id FROM user_wallets WHERE user_id='2'");
  assert.equal(verified.rows[0].public_key,'a'.repeat(64));assert.equal(verified.rows[0].wallet_version,'W5');assert.equal(verified.rows[0].wallet_id,'2147483648');
  await assert.rejects(db.exec("UPDATE user_wallets SET public_key='not-a-key' WHERE user_id='2'"));
  await assert.rejects(db.exec("UPDATE user_wallets SET wallet_id='invalid' WHERE user_id='2'"));
